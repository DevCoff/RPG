...
function calcularDano(personagem, oponente, habilidade, tipoAtaque) {
    ...
    const danoBase = (2 * personagem.lvl + 10) / 250 * (personagem.ataque / oponente.defesa) * habilidade.poder + 2; // Calculo do dano base
...
// Função de ataque
export async function ataque(personagem, oponente, habilidadeEscolhida) { // ela recebe 3 parametros, personagem, oponente e a habilidade escolhida, pois no jogo vc seleciona a habilidade
    const habilidade = personagem.habilidades.find(hab => hab.nome === habilidadeEscolhida); // essa variavel procura a habilidade pelo nome
    if (!habilidade) { // se a habilidade escolhida nao estiver nas habilidades, irá retornar a mensagem abaixo
        await digitarTexto(chalk.red(`${personagem.nome} não possui a habilidade ${habilidadeEscolhida}.`));
        return;
    }
    ...
    switch (habilidade.tipo) { // aqui vai verificar o tipo da habilidade usada, cada uma tem o seu tipo.
        case 'Ataque':
        ...
        case 'Especial': // a habilidade Especial eu decidi por que aumentaria entre 0 ou 4, novamente usei o mesmo calculo do jogo Pokemon
            ...
            danoCalculado = calcularDano(personagem, oponente, habilidade, 'Especial'); // chama a função de cálculo de dano
            mensagem = `${personagem.nome} usa ${habilidade.nome} causando ${danoCalculado.toFixed(2)} de dano a ${oponente.nome}.`;
            break;
        default:
