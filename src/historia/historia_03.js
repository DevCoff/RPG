...
export async function historia3(heroi) {
    ...
    if (heroi.lvl <= 3) {
        ...
        await digitarTexto(chalk.bold('Narrador:\n'));
        await digitarTexto(chalk.bold(`${chalk.cyan(heroi.nome)} escuta um riso, o Dragão estava rindo de toda a situação.`));
        await digitarTexto(chalk.bold(`Ele avança em direção ao Dragão, sem medo do inimigo.`));
        await digitarTexto(chalk.bold(`Bastou um balançar de asas e ${chalk.cyan(heroi.nome)} caiu em desespero.\n`));
        ...
        await digitarTexto(chalk.bold(`Naquele momento ${chalk.cyan(heroi.nome)} jurou se vingar, e um dia eles iriam se reencontrar.\n`));
        prompt(chalk.green('\n\nAPERTE ENTER PARA CONTINUAR...'))
    } else if (heroi.lvl > 3) {
        await digitarTexto(chalk.bold('Narrador:'));
        await digitarTexto(chalk.bold(`\n${chalk.cyan(heroi.nome)} estava em sua melhor forma, capaz de localizar o inimigo a quilômetros de distância.`));
        await digitarTexto(chalk.bold(`Mais rápido que um piscar de olhos, mais veloz que o estrondo do trovão, ${chalk.cyan(heroi.nome)} se lançou à frente de ${serena}, desviando-a da rajada de energia que a atingiria.`));
        await digitarTexto(chalk.bold(`${chalk.cyan(heroi.nome)} estava gravemente ferido, mas ainda de pé, ele encarou o dragão e bradou...\n`));
        await digitarTexto(chalk.bgBlackBright(`${heroi.nome}:\n`));
        ...
        await digitarTexto(chalk.bgRed('Dragão Acnologia:\n'));
        await digitarTexto(chalk.bold(`Lembrarei de você ${chalk.cyan(heroi.nome)}. Estarei esperando na Ilha das Chamas pelo seu desafio. Prepare-se, torne-se mais forte e venha me enfrentar.\n`));
        await digitarTexto(chalk.bold('Narrador:\n'));
        ...
        await digitarTexto(chalk.bold(`Um tempo depois, ${chalk.cyan(heroi.nome)} já havia se recuperado.\n`))
        await digitarTexto(chalk.bgCyan(`${serena}:\n`));
        await digitarTexto(chalk.bold(`${chalk.cyan(heroi.nome)}, estamos quase lá. Olhe, quero te apresentar à minha família.\n`));
        await digitarTexto(chalk.bold('Narrador:\n'));
