...
export async function torre(heroi) {
  // recebe como parametro o Heroi, pois os dados precisam ser carregados em cada historia
  ...
  await digitarTexto(chalk.bold("Narrador:\n"));
  await digitarTexto(
    chalk.bold(`${chalk.cyan(heroi.nome)}sobe para o próximo andar.`),
  );
  await digitarTexto(
    chalk.bold("Novamente aparece seu nome e seu próximo adversário."),
  );
  ...
  await digitarTexto(
    chalk.bold(
      `Após uma dura batalha, ${chalk.cyan(heroi.nome)} olha para a tela e vê um garoto que tinha pelo menos 12 anos em pé no ringue e pensa.`,
    ),
  );
  await digitarTexto(chalk.bgBlackBright(`${heroi.nome}:\n`));
  await digitarTexto(
    chalk.underline(
      `Aquele garoto está subindo a torre, vencendo seus adversários com somente um golpe...`,
    ),
  );
  await digitarTexto(chalk.bold("Narrador:\n"));
